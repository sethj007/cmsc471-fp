...
Object.entries(legendRegions).forEach(([region, color]) => {
    ...
    item.addEventListener("click", () => {
        if (activeRegions.has(region)) { activeRegions.delete(region); item.classList.add("inactive"); }
        else { activeRegions.add(region); item.classList.remove("inactive"); }
...
// === DATA BUTTON → scroll to data section on intro tab ===
